...
path_test = 'd:/stockdata/test_data/'
path_down = 'd:/stockdata/period_down/'
path_depress = 'd:/stockdata/depress/'
...
def kospi_kosdaq(lastday='20251231', market='1001'):
    market_name = 'kospi' if market == '1001' else 'kosdaq'
    df = get_index_ohlcv_by_date(kospi_next_day_no_hypyen, lastday, market)
    df.index.names = ['Date']
    df = df.iloc[:,[0,1,2,3,4]]
    df.columns  = ['Open','High','Low','Close','Volume']
    df['Market'] = market_name
    df.to_sql(name=market_name, con=engine, if_exists='append')
...
def day_week_month_data(market='kospi', from_day='2020-01-01', to_day=str_today, period='month'):
    ...
    if period == 'month':
        period = [g for n, g in df.groupby(pd.Grouper(key='Date', freq='M'))]
        
    elif period == 'week':
        period = [g for n, g in df.groupby(pd.Grouper(key='Date', freq='W'))]

    rows = []
    for p in period:
        rows.append(p.iloc[-1]['Date'])
        rows.append(p.iloc[0]['Open'])
        rows.append(p['High'].max())
        rows.append(p['Low'].min())
        rows.append(p.iloc[-1]['Close'])
        rows.append(p['Volume'].sum())
    ...
    if period == 'day':
        ...
        arr1 = arr.reshape(len(period), 6) if period == 'month' else arr.reshape(per_week, 6)
        df = pd.DataFrame(data=arr1, columns=['Date', 'Open', 'High', 'Low', 'Close', 'Volume'])
...
def depress(period='day', to_day=str_today):
    ...
    from_day_dict = {'day': 60, 'week': 180, 'month': 365*2}

    try:
        from_day = (pd.to_datetime(to_day) - timedelta(days=from_day_dict[period])).strftime('%Y-%m-%d')
    except KeyError:
        print("depress( ['day', 'week', 'month'] )  ")
        return

    df = select_stock('all', stock_last_day, stock_last_day)['Name']
    name = df.to_list()
    ...
    for i in name:
        ...
        df['minus'] = (df['Close']-df['Close'].shift(1)) < 0
    ...
    df2 = pd.DataFrame({'name': name, 'count': depress}).sort_values(by=['count'], axis=0, ascending=False, ignore_index=True)
    if period == 'month':
        df3 = df2.iloc[:100]
    elif period == 'week':
        df3 = df2.iloc[:200]
...
def make_name_list(path_name='d:/stockdata/depress/', arg='*day*.*', num=0, degree=30):
...
def ma(df):
    ...
    for p in periods:
        ma_name = f'ma{p}'
        df[ma_name] = ta.MA(df, timeperiod=p)
